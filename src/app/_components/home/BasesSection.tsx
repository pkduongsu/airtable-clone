              ...
              <div key={base.id} className="relative group h-[92px] w-[300px] flex-shrink-0">
                      ...
                      <div className="flex">
                        <div className="flex items-center justify-center rounded-l-lg relative w-[92px] h-[92px] min-w-[92px]">
                          <div className="flex justify-center items-center w-[56px] h-[56px] rounded-[12px] relative bg-[#63498d] text-white">
                              {/* TODO: If Untitled -> Un, else other svg */}
                              <span className="font-family-system font-[400] text-[22px] leading-[18px] ">Un</span>
                          </div>
                        </div>
                        <div className="flex flex-col flex-auto justify-center">
                          <h3 className="font-medium text-sm mb-0.5">{base.name}</h3>
                          <p className="text-xs text-gray-500">
                            {getRelativeTime(base.updatedAt)}
                          </p>
                        </div>
                      </div>
                  </div>
                ...
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    onDeleteBase(base.id, base.name);
                  }}
                  className="absolute top-2 right-2 p-1.5 rounded-[6px] bg-white shadow-at-main-nav border border-gray-200 text-gray-500 hover:text-red-600 hover:border-red-200 opacity-0 group-hover:opacity-100 transition-all duration-200 z-10"
                  title="Delete base"
                >
                  <Trash2 size={14} />
                </button>
          ...
          <div className="flex flex-col w-full items-start gap-4 mb-6">
            {bases?.map((base) => (
              <div key={base.id} className="relative group w-full max-w-[572px]">
                <Link href={`/${base.id}`} className="block">
                  <div 
                    className="relative rounded-[6px] bg-white cursor-pointer shadow-at-main-nav h-[92px] w-full"
                  >
                      <div className="flex">
                        <div className="flex items-center justify-center rounded-l-lg relative w-[92px] h-[92px] min-w-[92px]">
                          <div className="flex justify-center items-center w-[56px] h-[56px] rounded-[12px] relative bg-[#63498d] text-white">
